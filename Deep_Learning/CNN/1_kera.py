...
# 輸入的x變成 60,000*784 的數據，然後除以255進行標準化，每個像素都
# 是在0到255之間的 (顏色階數從0~255)，標準化之後就變成了0到1之間。
x_train /= 255
x_test /= 255
#把lebel變成one-hot的形式
...
# 加入dropout隨機在訓練時關閉輸入與權重的影響, 防止過擬合 ,0.2表示有2成輸入被關閉
...
# 損失函數用交叉熵crossentropy
#優化器用RMSprop
#metrics，可放入需要評估的cost，accuracy，score等
model.compile(loss='categorical_crossentropy',
optimizer=RMSprop(),
metrics=['accuracy'])  # <-需要觀察的資訊
#將epoch的訓練結果保存在csv文件中
logger = CSVLogger('model_KerasMnistMLP.log')
#當監測值val_loss不再改善時，如發現損失沒有下降，則經過3次epoch後停止訓練。
estop = EarlyStopping(monitor='val_loss', patience=3) # <-試著改變patience
